...
const account3 = {
  owner: 'Afraa Alshikh',
  movements: [888888888, 65656566, -150, -790, 3210, 565565656, 8500, -30],
  interestRate: 1.5,
  pin: 3333,
...
const formatMovements = function (date) {
  const clacDaysPassed = function (day1, day2) {
    return Math.round(Math.abs(day1 - day2) / (1000 * 60 * 60 * 24));
  };

  const datebet = clacDaysPassed(date, new Date());
  if (datebet === 0) return 'Today';
  if (datebet === 1) return 'Yesterday';
  if (datebet <= 7) return `${datebet} days ago`;

  return new Intl.DateTimeFormat(currentAccount.locale).format(date);
};

const formatedMov = function (value, locale, currency) {
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumIn.textContent = `${formatedMov(incomes, acc.locale, acc.currency)}`;

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${formatedMov(
    Math.abs(out),
    acc.locale,
    acc.currency
  )}`;
  ...
  labelSumInterest.textContent = `${formatedMov(
    interest,
    acc.locale,
    acc.currency
  )}`;
